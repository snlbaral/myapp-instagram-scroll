...
export class SelectedElement extends PureComponent {
  ...
  render() {
    ...
    let imageIndex = children.props.currentImageEmitter();
    let ImageComponent = children.props.ImageComponent;
    let imageComponentStyle = children.props.ImageComponentStyle;
    let image = children.props.images[imageIndex || 0];

    return (
      <View style={styles.root}>
        {renderBackground(selected, scaleValue, gesturePosition)}
        <Animated.View style={elementStyle}>
          <ImageComponent style={imageComponentStyle} source={{ uri: image }} />
        </Animated.View>
      </View>
    );
  }
